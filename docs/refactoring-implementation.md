...
Main advanced again to `8ab4c35` (Incognito windows and the read-only Hermes plugin). The integration gives each in-memory database its own serial queue and history SQL functions, preserves private gateway process-group cleanup in the extracted transport, and completes structured requests with an error after a private window closes. Incognito native tests include a regression for the latter callback path.

Final validation after that integration: **37 native suites**, **137 discovered Python tests**, 13 agent-runtime tests, 7 terminal-service tests, 146 browser checks, browser-profile import, and theme audit pass. The signed desktop Incognito browser probe verifies isolation between normal and private windows, between separate private windows, sharing within one private window, and absence of private cookie/localStorage markers on disk. The desktop app builds and passes strict signature verification. The previously reproduced QuickInputTests assertion remains excluded.
