...
static NSString *TLMarkdownHTML(NSString *text, TLThemePalette *palette, NSColor *textColor, NSFont *baseFont, BOOL rendersMarkdown) {
  ...
  return [NSString stringWithFormat:
    ...
    @".tl-table-scroll{box-sizing:border-box;max-width:100%%;overflow-x:auto;margin:0 0 %.1fpx 0;}"
    @"table{box-sizing:border-box;width:100%%;border-collapse:collapse;table-layout:auto;}"
    // Inherited overflow-wrap:anywhere reduces a column's intrinsic minimum to a
    // single character. Preserve whole words and scroll tables that cannot fit.
    @"th,td{border:%.1fpx solid %@;padding:%.1fpx %.1fpx;text-align:left;vertical-align:top;overflow-wrap:normal;}"
    ...
    @"if(!md){md=window.markdownit({html:false,linkify:true,typographer:false,breaks:false});"
    ...
    @"const scroll=document.createElement('div');scroll.className='tl-table-scroll';scroll.tabIndex=0;"
    @"scroll.setAttribute('role','region');scroll.setAttribute('aria-label','Table');"
