...
@property NSPopUpButton *category;
...
- (void)buildContent {
  ...
  self.category = [[NSPopUpButton alloc] init]; [self.category addItemsWithTitles:TLBrowserPreferences.categories];
  self.category.font = self.palette.bodyFont; self.category.target = self; self.category.action = @selector(filter:);
  self.category.accessibilityLabel = @"Browser settings category";
  ...
  NSStackView *content = [self stack:@[self.search,self.category,self.status,self.rows]];
...
- (void)filter:(id)sender {
  ...
  for (NSDictionary *setting in TLBrowserPreferences.catalogue) {
    ...
    BOOL show = query.length ? [text rangeOfString:query options:NSCaseInsensitiveSearch].location != NSNotFound : [setting[@"category"] isEqual:self.category.titleOfSelectedItem];
    self.cards[setting[@"id"]].hidden = !show; if (show) count++;
  }
  for (NSDictionary *row in self.extraRows) {
    BOOL show = query.length ? [row[@"search"] rangeOfString:query options:NSCaseInsensitiveSearch].location != NSNotFound : [row[@"category"] isEqual:self.category.titleOfSelectedItem];
    [row[@"row"] setHidden:!show]; if (show) count++;
  }
  self.empty.hidden = count > 0; self.category.enabled = !query.length;
}
