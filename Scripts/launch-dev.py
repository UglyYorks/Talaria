...
def snapshot_database(source, destination, data_root):
    ...
    try:
        with closing(sqlite3.connect(source.as_uri() + "?mode=ro", uri=True)) as original:
            with closing(sqlite3.connect(destination)) as copied:
                ...
                # Preserve conversations/settings, but never start or delete the
                # original VMs or inherit write access to the user's folders.
                columns = {row[1] for row in copied.execute("PRAGMA table_info(agents)")}
                if columns:
                    for (agent_id,) in copied.execute("SELECT id FROM agents").fetchall():
                        copied.execute("UPDATE agents SET vm_directory=?, status='stopped', last_error=NULL WHERE id=?",
                                       (str(data_root / "Agents" / str(agent_id)), agent_id))
                    if "folder_paths" in columns:
                        copied.execute("UPDATE agents SET folder_paths='[]'")
                copied.commit()
...
def launch(copy_db, instructions, root=ROOT):
    ...
    with (development / "launcher.lock").open("w") as lock:
        ...
        try:
            ...
            snapshot_database(source, data / "talaria.sqlite3", data)
            ...
            # Reuse the build's identity rather than altering the installed helper.
...
def main():
    ...
    parser.add_argument("--copy-db", action="store_true", required=True,
                        help="snapshot the current database into this worktree's gitignored .talaria-dev folder")
